...
class ResearchWorkspaceConsumerProjectionExecutionCapabilityRegistryEventSubscriptionLifecyclePolicyProfileBindingTemplateDeploymentHistoryService:
    """
    Records and queries the deployment history of consumer
    projection execution capability registry event subscription
    lifecycle policy profile binding templates, including the
    bindings instantiated by each deployment, for auditing, rollback,
    and operational analysis.

    The service's responsibility is recording and read-only
    querying, not deployment itself, template creation, or
    membership management. It does NOT deploy templates, create
    templates, mutate template membership, mutate a recorded record,
    ...
    def __init__(self):
        self._history = ResearchWorkspaceConsumerProjectionExecutionCapabilityRegistryEventSubscriptionLifecyclePolicyProfileBindingTemplateDeploymentHistory(
            records=()
        )
    ...
    def record(
        self,
        deployment_record: ResearchWorkspaceConsumerProjectionExecutionCapabilityRegistryEventSubscriptionLifecyclePolicyProfileBindingTemplateDeploymentRecord,
    ) -> None:
        """
        Record a deployment.
        ...
        Raises:
            ResearchWorkspaceConsumerProjectionExecutionCapabilityRegistryEventSubscriptionLifecyclePolicyProfileBindingTemplateDeploymentHistoryError:
                ...
                ResearchWorkspaceConsumerProjectionExecutionCapabilityRegistryEventSubscriptionLifecyclePolicyProfileBindingTemplateDeploymentRecord,
                has a blank deployment ID, template ID, environment,
                or version, has None instantiated binding IDs, has an
        ...
        with self._lock:
            if any(
                ...
                raise ResearchWorkspaceConsumerProjectionExecutionCapabilityRegistryEventSubscriptionLifecyclePolicyProfileBindingTemplateDeploymentHistoryError(
    ...
    def history(self, template_id: str) -> tuple:
        """
        List every recorded deployment for a template, in
        chronological order.
        """

        with self._lock:
            return tuple(
                record for record in self._history.records if record.template_id == template_id
            )
    ...
    def latest(self, template_id: str):
        """
        Find the most recently recorded deployment for a template.

        Returns:
            The most recent deployment record for template_id, or
            None if no deployment has ever been recorded for it
        """

        matching = self.history(template_id)
    ...
    def _validate_record(self, record) -> None:
        ...
        if record.template_id is None or not record.template_id.strip():
            raise ResearchWorkspaceConsumerProjectionExecutionCapabilityRegistryEventSubscriptionLifecyclePolicyProfileBindingTemplateDeploymentHistoryError(
                "Cannot record a deployment with an empty or blank template ID."
            )
