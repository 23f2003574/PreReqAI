...
class ResearchWorkspaceConsumerProjectionExecutionCapabilityRegistryEventSubscriptionLifecyclePolicyTemplateParameterValues:
    """
    Immutable, order-preserving set of parameter values for a
    consumer projection execution capability registry event
    subscription lifecycle policy template's parameter set.

    The values are a value object only. They perform no validation,
    no defaulting, and no application. Validation, defaulting, and
    application are the responsibility of a parameterization
    service.
